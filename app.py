...
def calculate_scores(before_7_am_japa_session, before_7_am, from_7_to_9_am, after_9_am, book_reading_time_min, lecture_time_min, seva_time_min):
    ...
    # Ensure book_reading_time_min is a list for pd.cut
    score_b = pd.cut([book_reading_time_min], bins=[-1, 1, 15, 30, 45, 60, float('inf')], labels=[0, 7, 15, 20, 25, 30], ordered=False).astype(float).fillna(0).astype(int)[0]
    
    # Ensure lecture_time_min is a list for pd.cut
    score_c = pd.cut([lecture_time_min], bins=[-1, 15, 30, 45, float('inf')], labels=[7, 15, 20, 30], ordered=False).astype(float).fillna(0).astype(int)[0]
    
    # Ensure seva_time_min is a list for pd.cut
    score_d = pd.cut([seva_time_min], bins=[-1, 1, 15, 30, 45, 60, float('inf')], labels=[0, 5, 8, 12, 14, 15], ordered=False).astype(float).fillna(0).astype(int)[0]
